...
const Label = ({ children }) => (
  <label className="block text-xs font-semibold text-orange-400 uppercase tracking-wide mb-1">{children}</label>
)
...
export function CreateListingPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
      <h1 className="font-display font-extrabold text-2xl sm:text-3xl mb-6 sm:mb-8">Publicar vivienda</h1>
      ...
      <form onSubmit={handleSubmit} className="card space-y-5 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
          <div className="sm:col-span-2">
            <Label>Título *</Label>
            <input className="input" value={form.title} onChange={setField('title')}
              placeholder="Ej: Dpto 2 hab. en Asunción" required />
          </div>
          <div>
            <Label>Tipo</Label>
          ...
          <div>
            ...
            <input className="input" type="number" value={form.total_price} onChange={setField('total_price')}
              placeholder="Ej: 2500000" required />
          </div>

          {/* Numeric inputs — 3 per row on sm+, individual on xs */}
          <div>
          ...
          <div>
            ...
            <input className="input" type="number" step="any" value={form.latitude}
              onChange={setField('latitude')} placeholder="-25.2867" />
          </div>
          <div className="sm:col-span-2">
        ...
        <div>
          ...
          {/* Responsive grid: 3 cols on xs, 4 on sm+ */}
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3 mt-2">
            ...
            {photoFiles.length < 8 && (
              <label className="aspect-square rounded-xl border-2 border-dashed border-orange-200
                flex flex-col items-center justify-center cursor-pointer
                hover:border-orange-400 hover:bg-orange-50 transition">
                <ImagePlus size={20} className="text-orange-300" />
          ...
          <p className="text-xs text-gray-400 mt-2">Las fotos se suben al publicar.</p>
        </div>
