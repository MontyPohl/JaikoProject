...
export default function SearchPage() {
  ...
  const [filters, setFilters] = useState({ city: 'Asunción', pets: '', smoker: '', schedule: '', min_age: '', max_age: '' })
  ...
  const fetchProfiles = useCallback(async (p = 1, currentFilters = filters) => {
    ...
    try {
      ...
      // Enviamos "true"/"false" — el backend los acepta y también "1"/"0" tras el fix
  ...
  // Dispara búsqueda ante cualquier cambio de filtro; debounce 400ms solo en campos numéricos
  useEffect(() => {
    const timer = setTimeout(() => { fetchProfiles(1, filters) }, filters.min_age || filters.max_age ? 400 : 0)
    return () => clearTimeout(timer)
  }, [filters]) // eslint-disable-line react-hooks/exhaustive-deps

  const handleSearch = (e) => { e.preventDefault(); fetchProfiles(1, filters) }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-orange-400 text-sm mt-1">
            {profiles.length > 0 ? `${profiles.length} roomies compatibles` : 'Perfiles compatibles con vos'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowFilters((f) => !f)} className="flex items-center gap-2 btn-secondary py-2 px-4 text-sm">
            <SlidersHorizontal size={16} /> Filtros {showFilters && <X size={14} />}
          </button>
          <button onClick={() => setShowMap((prev) => !prev)} className="flex items-center gap-2 btn-secondary py-2 px-4 text-sm">
      ...
      {showFilters && (
        <form onSubmit={handleSearch} className="card mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input type="number" className="input" placeholder="18" min={18} max={80} value={filters.min_age} onChange={(e) => setFilters((f) => ({ ...f, min_age: e.target.value }))} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-orange-400 mb-1 uppercase tracking-wide">Edad máx.</label>
            <input type="number" className="input" placeholder="99" min={18} max={99} value={filters.max_age} onChange={(e) => setFilters((f) => ({ ...f, max_age: e.target.value }))} />
          </div>
          <div className="col-span-2 flex items-end justify-between gap-3">
            <button type="button" onClick={() => setFilters({ city: 'Asunción', pets: '', smoker: '', schedule: '', min_age: '', max_age: '' })} className="btn-ghost text-sm">
              Limpiar filtros
            </button>
      ...
      {loading && profiles.length === 0 ? (
        ...
        <EmptyState icon="🔍" title="No encontramos roomies compatibles" description="Probá ajustar tus preferencias en tu perfil o cambiar el filtro de ciudad." />
      ) : showMap ? (
            ...
            <Marker key={p.user_id} position={[p.location.lat, p.location.lng]}>
              <Popup>
                <div className="cursor-pointer w-60 p-2" onClick={() => navigate(`/profile/${p.user_id}`)}>
                  ...
                  <p className="text-xs text-gray-400 mb-1">{p.smoker ? 'Fumador' : 'No fuma'}{p.schedule ? ` • ${SCHEDULE_LABELS[p.schedule] || p.schedule}` : ''}</p>
        ...
        <>
          ...
          {hasMore && (
            <div className="flex justify-center mt-10">
              <button onClick={() => fetchProfiles(page + 1, filters)} disabled={loading} className="btn-secondary flex items-center gap-2">
